...
if (dtk_server)then {
	call compile preprocessFile "\MPMissions\tlx_server\functions\pre_init.sqf";
}else{
...
private ["_modules","_functions","_fn"];
dtk_fnc_total = 0;
...
/*loads module functions*/
{
	...
	_module = _x;
	{
		...
		missionNamespace setVariable [_fn,compile preprocessFile format["functions\%1\%2.fnc",_module,_fn]];
